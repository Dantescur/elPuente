<template>
  <div class="p-4 sm:p-6 lg:p-8 max-w-6xl">
    ...
    <div class="flex items-start justify-between gap-3 mb-6 flex-wrap sm:flex-nowrap">
      ...
      <button
class="flex items-center gap-2 px-4 py-2 text-sm font-semibold bg-violet-600 hover:bg-violet-500 text-white rounded-lg transition-colors shrink-0"
        @click="abrirNuevo">
...
v-model="filtroCategoria"
    ...
    <div v-else-if="productosFiltrados.length" class="md:hidden space-y-2">
      <div
v-for="p in productosFiltrados" :key="p.id" class="bg-zinc-900 border border-zinc-800 rounded-xl p-4 active:scale-[0.99] transition-transform cursor-pointer hover:border-zinc-700"
        @click="abrirEditar(p)">
        <div class="flex items-start justify-between gap-3">
          ...
          <div class="text-right shrink-0">
            <p class="mono text-base font-semibold" :class="stockColor(p)">{{ p.stock }}</p>
...
v-for="p in productosFiltrados" :key="p.id"
            ...
            <td class="td"><span class="mono text-sm font-medium" :class="margenColor(p)">{{ calcMargen(p) }}%</span>
            </td>
            <td class="td">
...
class="p-1.5 text-zinc-500 hover:text-zinc-100 hover:bg-zinc-700 rounded-md transition-all"
    ...
    <div v-else class="py-16 text-center text-zinc-600">
      <svg
class="w-10 h-10 mx-auto mb-3 opacity-30" viewBox="0 0 24 24" fill="none" stroke="currentColor"
        stroke-width="1.5">
        <path
          ...
          <Transition name="sheet">
            <div
              ...
              <div class="flex items-center justify-between px-5 py-4 border-b border-zinc-800">
                <h2 class="text-base font-bold tracking-tight">{{ editando ? 'Editar producto' : 'Nuevo producto' }}
                </h2>
                <button
              ...
              <div class="p-5 space-y-4 max-h-[70dvh] overflow-y-auto">
                ...
                <div class="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
v-model.number="form.costo" type="number" min="0" step="0.01" class="field-input"
                      placeholder="0.00" >
                  </div>
                  <div>
                    <label class="field-label">Precio de venta *</label>
                    <input
v-model.number="form.precio_venta" type="number" min="0" step="0.01" class="field-input"
                      placeholder="0.00" >
                  </div>
                </div>
...
v-if="form.costo > 0 && form.precio_venta > form.costo"
                  ...
                  <svg
class="w-3.5 h-3.5 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                    stroke-width="2.5">
                    <polyline points="23 6 13.5 15.5 8.5 10.5 1 18" />
                    <polyline points="17 6 23 6 23 12" />
                  </svg>
                  <span class="mono font-medium">{{ ((form.precio_venta - form.costo) / form.precio_venta *
                    100).toFixed(1) }}% margen</span>
...
class="w-3.5 h-3.5 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                ...
                <div class="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <input
v-model.number="form.stock_minimo" type="number" min="0" class="field-input"
                      placeholder="5" >
                  </div>
                </div>
                <div v-if="editando">
                  <label class="field-label">Motivo de ajuste</label>
                  <input v-model="form.motivo" class="field-input" placeholder="Ej: Reposición..." >
                </div>
                <div>
...
class="flex-1 py-2.5 text-sm font-semibold text-zinc-400 bg-zinc-800 hover:bg-zinc-700 rounded-lg transition-colors"
                ...
                <button
:disabled="guardando || !formValido" class="flex-1 py-2.5 text-sm font-semibold text-white bg-violet-600 hover:bg-violet-500 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                  @click="guardar">
                  <span
v-if="guardando"
...
interface Producto { id: number; nombre: string; costo: number; precio_venta: number; stock: number; stock_minimo: number; categoria: string; descripcion?: string | null }

const { data: raw, pending: cargando, refresh } = await useFetch('/api/productos')
const productos = computed<Producto[]>(() => raw.value?.data ?? [])
const busqueda = ref(''); const filtroCategoria = ref('')
...
const modalAbierto = ref(false); const editando = ref<Producto | null>(null); const guardando = ref(false)
const formDef = () => ({ nombre: '', costo: 0, precio_venta: 0, stock: 0, stock_minimo: 5, categoria: '', descripcion: '', motivo: '' })
const form = ref(formDef())
const formValido = computed(() => form.value.nombre && form.value.costo > 0 && form.value.precio_venta > form.value.costo && form.value.categoria)
...
async function guardar() {
  if (!formValido.value) return; guardando.value = true
  try {
    ...
    cerrarModal(); await refresh()
  } catch (e: unknown) { 
    const error = e as { data?: { message?: string } } | Error
    const message = typeof error === 'object' && error !== null && 'data' in error ? (error as any).data?.message : undefined
...
const fmt = (n: number | string | null | undefined): string => 
  new Intl.NumberFormat('es-ES', { style: 'currency', currency: 'CUP', maximumFractionDigits: 0 }).format(Number(n) || 0)
...
function toast(msg: string, type = 'success') {
  const id = tid++; toasts.value.push({ id, msg, type })
