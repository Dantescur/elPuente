<template>
  <div class="p-4 sm:p-6 lg:p-8 xl:p-10">
    ...
    <div
      class="grid md:grid-cols-[1fr_300px] lg:grid-cols-[1fr_360px] xl:grid-cols-[1fr_400px] gap-4 xl:gap-6 items-start">
      ...
      <div class="bg-zinc-900 border border-zinc-800 rounded-xl p-4 sm:p-5">
        ...
        <div class="flex gap-1.5 flex-wrap mb-3">
          <button v-for="c in ['Todos', ...categorias]" :key="c"
            class="px-3 py-1 rounded-full text-xs font-medium transition-all border" :class="catActiva === c
        ...
        <div v-else
          class="grid grid-cols-2 sm:grid-cols-3 gap-2 max-h-[48dvh] md:max-h-[calc(100dvh-340px)] overflow-y-auto pr-0.5">
          <button v-for="p in productosFiltrados" :key="p.id" :disabled="p.stock === 0"
            class="relative text-left flex flex-col gap-1 p-3 rounded-xl border transition-all" :class="[
              p.stock === 0
      ...
      <div class="space-y-3">
        <div class="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
          ...
          <div v-if="!carrito.length" class="py-10 text-center">
            <svg class="w-7 h-7 mx-auto mb-2 text-zinc-700" viewBox="0 0 24 24" fill="none" stroke="currentColor"
              stroke-width="1.5">
            ...
            <div v-for="item in carrito" :key="item.producto.id" class="px-3 py-2.5 space-y-2">
              <div class="flex items-center gap-2">
                ...
                <div class="flex items-center gap-1 shrink-0">
                  ...
                  <input :value="item.cantidad" type="number" min="1" :max="item.producto.stock"
                  ...
                  <button :disabled="item.cantidad >= item.producto.stock"
                ...
                <button class="text-zinc-600 hover:text-red-400 transition-colors p-0.5 shrink-0"
              ...
              <input v-model="item.motivo" placeholder="Motivo (opcional)"
        ...
        <div v-if="ultimasVentas.length" class="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
          <p
            class="text-[10px] font-semibold uppercase tracking-widest text-zinc-500 px-4 py-3 border-b border-zinc-800">
            Últimas ventas
          </p>
