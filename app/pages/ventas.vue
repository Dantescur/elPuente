...
v-model="busqueda" placeholder="Buscar..."
        ...
        <div class="flex gap-1.5 flex-wrap mb-3">
          <button
v-for="c in ['Todos', ...categorias]" :key="c" class="px-3 py-1 rounded-full text-xs font-medium transition-all border"
            :class="catActiva === c
...
v-for="p in productosFiltrados" :key="p.id"
            ...
            :class="[
              p.stock === 0 ? 'opacity-40 cursor-not-allowed bg-zinc-800/30 border-zinc-800' :
              enCarrito(p) ? 'border-violet-500/60 bg-violet-500/8 cursor-pointer' :
            ...
            <span class="text-[10px]" :class="p.stock === 0 ? 'text-red-500' : p.stock <= p.stock_minimo ? 'text-amber-400' : 'text-zinc-600'">
              {{ p.stock === 0 ? 'Sin stock' : `${p.stock} disp.` }}
            </span>
            <!-- Cart check -->
            <svg v-if="enCarrito(p)" class="absolute top-2 right-2 w-3.5 h-3.5 text-violet-400" viewBox="0 0 24 24" fill="currentColor">
      ...
      <div class="space-y-3">
        ...
        <div class="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
          <div class="flex items-center justify-between px-4 py-3 border-b border-zinc-800">
            ...
            <span v-if="carrito.length" class="mono text-[11px] bg-violet-500/15 text-violet-400 px-2 py-0.5 rounded-full">
          ...
          <div v-else class="divide-y divide-zinc-800">
            <div
v-for="item in carrito" :key="item.producto.id"
...
class="w-6 h-6 rounded-md border border-zinc-700 text-zinc-400 hover:text-zinc-100 hover:border-zinc-500 transition-all flex items-center justify-center text-base leading-none"
                ...
                <button
:disabled="item.cantidad >= item.producto.stock" class="w-6 h-6 rounded-md border border-zinc-700 text-zinc-400 hover:text-zinc-100 hover:border-zinc-500 disabled:opacity-30 transition-all flex items-center justify-center text-base leading-none"
                  @click="cambiarCantidad(item, 1)">+</button>
              </div>
              <span class="mono text-sm text-zinc-300 w-14 text-right shrink-0">{{ fmt(item.producto.precio_venta * item.cantidad) }}</span>
        ...
        <div v-if="carrito.length" class="bg-zinc-900 border border-violet-500/20 rounded-xl p-4 space-y-3">
          ...
          <button
:disabled="procesando" class="w-full flex items-center justify-center gap-2 py-3 text-sm font-bold text-white bg-violet-600 hover:bg-violet-500 rounded-xl transition-colors disabled:opacity-50"
            @click="procesarVenta">
...
class="w-full py-2 text-sm font-medium text-zinc-500 hover:text-zinc-300 hover:bg-zinc-800 rounded-lg transition-all"
        ...
        <div v-if="ultimasVentas.length" class="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
          <p class="text-[10px] font-semibold uppercase tracking-widest text-zinc-500 px-4 py-3 border-b border-zinc-800">Últimas ventas</p>
          <div class="divide-y divide-zinc-800/60">
            <div v-for="v in ultimasVentas" :key="v.id" class="flex items-center justify-between px-4 py-2.5">
              <div>
                ...
                <p class="mono text-[11px] text-zinc-500">×{{ v.cantidad }} · {{ fmt(v.subtotal) }}</p>
              </div>
    ...
    <!-- Toast -->
    <Teleport to="body">
      <div class="fixed bottom-24 md:bottom-6 right-4 flex flex-col gap-2 z-[100]">
        <TransitionGroup name="toast">
          <div
v-for="t in toasts" :key="t.id"
            class="flex items-center gap-3 px-4 py-3 rounded-xl bg-zinc-900 border text-sm shadow-2xl min-w-[220px]"
            :class="t.type === 'success' ? 'border-emerald-500/30' : 'border-red-500/30'">
            <div class="w-1.5 h-1.5 rounded-full shrink-0" :class="t.type === 'success' ? 'bg-emerald-400' : 'bg-red-400'"/>
            {{ t.msg }}
          </div>
        </TransitionGroup>
      </div>
...
interface Producto { id: number; nombre: string; costo: number; precio_venta: number; stock: number; stock_minimo: number; categoria: string }
interface CartItem { producto: Producto; cantidad: number }
interface VentaResult { id: number; producto: string; cantidad: number; subtotal: number; ganancia: number }

const { data: raw, pending: cargando } = await useFetch<any>('/api/productos')
const productos = computed<Producto[]>(() => raw.value?.data ?? [])
const busqueda = ref(''); const catActiva = ref('Todos')
...
const enCarrito = (p: Producto) => carrito.value.some(i => i.producto.id === p.id)
function agregarAlCarrito(p: Producto) {
  const e = carrito.value.find(i => i.producto.id === p.id)
  e ? (e.cantidad < p.stock && e.cantidad++) : carrito.value.push({ producto: p, cantidad: 1 })
}
function cambiarCantidad(item: CartItem, d: number) {
  const n = item.cantidad + d; if (n < 1) { quitarItem(item); return }; if (n > item.producto.stock) return; item.cantidad = n
}
...
const procesando = ref(false); const ultimasVentas = ref<VentaResult[]>([])
async function procesarVenta() {
  if (!carrito.value.length) return; procesando.value = true
  try {
    const res = await $fetch<any>('/api/ventas', { method: 'POST', body: carrito.value.map(i => ({ productoId: i.producto.id, cantidad: i.cantidad })) })
    ultimasVentas.value = [...res.data.ventas, ...ultimasVentas.value].slice(0, 8)
    toast(`Venta registrada · ${fmt(totalVenta.value)}`, 'success'); limpiarCarrito(); await refreshNuxtData()
  } catch (e: any) { toast(e?.data?.message || 'Error al procesar venta', 'error') }
