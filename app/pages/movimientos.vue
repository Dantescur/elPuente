    ...
    <div class="flex items-start justify-between gap-3 mb-6 flex-wrap sm:flex-nowrap">
      <div>
        ...
        <p class="text-xs font-mono text-zinc-500 mt-1">
          ...
          <NuxtTime v-if="lastUpdate" :datetime="lastUpdate" relative locale="es" />
        </p>
      </div>
...
class="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-zinc-500 pointer-events-none"
        ...
        <input
v-model="busquedaLocal" placeholder="Buscar por producto..."
...
v-model="filtros.tipo"
      ...
      <input
v-model="filtros.fecha_desde" type="date"
      ...
      <input
v-model="filtros.fecha_hasta" type="date"
        class="w-full px-3 py-2 text-sm bg-zinc-900 border border-zinc-800 rounded-lg text-zinc-100 focus:outline-none focus:border-violet-500 transition-colors">
    </div>
    ...
    <div v-else-if="movimientosFiltrados.length" class="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
      <div class="overflow-x-auto">
        <table class="w-full">
          ...
          <tbody>
            <tr
v-for="m in movimientosFiltrados" :key="m.id"
              ...
              <td class="td">
                ...
                <span class="text-xs font-mono text-zinc-400 block">
                  <NuxtTime :datetime="m.fecha" day="2-digit" month="short" year="numeric" />
                </span>
                <span class="text-[10px] text-zinc-600 block">
                  <NuxtTime :datetime="m.fecha" hour="2-digit" minute="2-digit" />
                </span>
              </td>
              <td class="td">
              ...
              <td class="td">
                <span class="badge" :class="tipoClass(m.tipo)">{{ tipoLabel(m.tipo) }}</span>
              </td>
              <td class="td">
              ...
              <td class="td"><span class="mono text-sm">{{ m.stock_anterior }}</span></td>
              <td class="td">
                <span class="mono text-sm font-semibold" :class="stockNuevoClass(m)">{{ m.stock_nuevo }}</span>
              </td>
...
class="p-1.5 text-zinc-500 hover:text-zinc-100 hover:bg-zinc-700 rounded-md transition-all"
    ...
    <div v-else class="py-20 text-center text-zinc-600">
      <svg
class="w-12 h-12 mx-auto mb-4 opacity-30" viewBox="0 0 24 24" fill="none" stroke="currentColor"
        stroke-width="1.5">
                ...
                <div class="grid grid-cols-2 gap-4">
                  ...
                  <div class="space-y-1">
                    ...
                    <p class="text-sm font-mono">
                      <NuxtTime
:datetime="movimientoSeleccionado.fecha" day="2-digit" month="short" year="numeric"
...
// Build query params from filters for server-side filtering
...
const movimientos = computed<Movimiento[]>(() => (raw.value as any)?.data ?? [])

const movimientosFiltrados = computed(() => {
  if (!busquedaLocal.value) return movimientos.value
  const q = busquedaLocal.value.toLowerCase()
  return movimientos.value.filter(m => m.producto?.nombre.toLowerCase().includes(q))
})

const resumen = computed(() =>
  movimientosFiltrados.value.reduce(
    (acc, m) => {
      if (m.tipo === 'entrada') acc.entradas += Math.abs(m.cantidad)
      else if (m.tipo === 'salida') acc.salidas += Math.abs(m.cantidad)
      else if (m.tipo === 'ajuste') acc.ajustes++
      return acc
    },
...
const lastUpdate = computed(() => {
  if (!movimientos.value.length) return null
  return new Date(Math.max(...movimientos.value.map(m => new Date(m.fecha).getTime()))).toISOString()
})

const detalleAbierto = ref(false)
const movimientoSeleccionado = ref<Movimiento | null>(null)
function verDetalle(m: Movimiento) { movimientoSeleccionado.value = m; detalleAbierto.value = true }

function limpiarFiltros() {
  filtros.value = { tipo: '', fecha_desde: '', fecha_hasta: '' }
  busquedaLocal.value = ''
}
...
function exportarCSV() {
  ...
  const rows = movimientosFiltrados.value.map(m => [
    m.fecha, m.producto?.nombre || '', m.producto?.categoria || '',
    tipoLabel(m.tipo), (m.tipo === 'salida' ? '-' : '+') + Math.abs(m.cantidad),
...
const tipoClass = (tipo: string) => ({ entrada: 'badge--green', salida: 'badge--red', ajuste: 'badge--violet' }[tipo] ?? 'badge--violet')
const tipoLabel = (tipo: string) => ({ entrada: 'Entrada', salida: 'Salida', ajuste: 'Ajuste' }[tipo] ?? tipo)
const cantidadClass = (m: Movimiento) => ({ entrada: 'text-emerald-400', salida: 'text-red-400', ajuste: 'text-violet-400' }[m.tipo] ?? 'text-zinc-100')
const stockNuevoClass = (m: Movimiento) => m.stock_nuevo === 0 ? 'text-red-400' : 'text-zinc-100'
