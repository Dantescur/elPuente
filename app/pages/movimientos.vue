...
// ✅ Filters that drive server-side API requests
const filtros = ref({ tipo: '', fecha_desde: '', fecha_hasta: '' })

// ✅ Local text search only (can't be done server-side without full-text search)
...
const { data: raw, pending: cargando, refresh } = await useFetch('/api/movimientos', {
  query: queryParams  // ✅ Reactive query params — refetches automatically when filters change
})
const movimientos = computed<Movimiento[]>(() => (raw.value as any)?.data ?? [])

// ✅ Only local text search runs client-side; tipo/date filtering is handled by the server
